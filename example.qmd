...
1. Make sure I am on the master branch by using the command : git branch. The terminal points to the branch it is currently on (i.e master)

2.Created an annotated tag using the command : git tag -a v1.0 -m "First stable version after merging testbranch"

This creates an annotated tag (-a), which includes a message (-m) and metadata like my name, email, and date.

3. Pushed the tag to GitHub : git push origin v1.0
